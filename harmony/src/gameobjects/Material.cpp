...
ofShader Material::shaderNoTexture;
ofShader Material::shaderTexture2D;
ofShader Material::shaderTextureRect;
bool Material::shadersInitialized = false;
size_t Material::shaderLights = 0;
...
void Material::setColors(ofFloatColor oDiffuse, ofFloatColor oAmbient, ofFloatColor oSpecular, ofFloatColor oEmissive) {
...
void Material::setDiffuseColor(ofFloatColor oDiffuse) {
	data.diffuse = oDiffuse;
}

void Material::setAmbientColor(ofFloatColor oAmbient) {
	data.ambient = oAmbient;
}

void Material::setSpecularColor(ofFloatColor oSpecular) {
	data.specular = oSpecular;
}

void Material::setEmissiveColor(ofFloatColor oEmissive) {
...
void Material::setData(const Material::Data &data) {
	this->data = data;
}
...
ofFloatColor Material::getDiffuseColor()const {
	return data.diffuse;
}

ofFloatColor Material::getAmbientColor()const {
	return data.ambient;
}

ofFloatColor Material::getSpecularColor()const {
	return data.specular;
}

ofFloatColor Material::getEmissiveColor()const {
...
void Material::initShaders(ofGLProgrammableRenderer & renderer) const {
...
void Material::updateMaterial(const ofShader & shader, ofGLProgrammableRenderer & renderer) const {
...
void Material::updateLights(const ofShader & shader, ofGLProgrammableRenderer & renderer) const {
...
#define SPECULAR_REFLECTION
	...
	void main (void){
		...
		// now add the material info
			...
			vec4 localColor = vec4(ambient,1.0) * mat_ambient + vec4(diffuse,1.0) * tex * mat_diffuse + vec4(specular,1.0) * mat_specular + mat_emissive;
		#else
